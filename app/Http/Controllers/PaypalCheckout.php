...
class PaypalCheckout extends Controller
{
  ...
  {
    return view('checkoutPaypal',compact('price'));
  }
  public function informations($price)
  {
    return view('checkoutDetailsPaypal',compact('price'));
  }

  public function informationsPost(Request $request)
  {
    $data = $request->only(['fullname', 'email', 'phone']);

    Mail::to('microdigitalesav@gmail.com')->send(new InformationsMail($data));
    return redirect()->route('chekoutIndex', ['price' => $request->price]);
